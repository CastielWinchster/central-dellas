...
export default function DriverVehicle() {
  ...
  const handleSave = async () => {
    ...
    if (yearNum && (yearNum < 1980 || yearNum > CURRENT_YEAR)) {
      toast.error(`Ano deve estar entre 1980 e ${CURRENT_YEAR}`);
      return;
    }
    ...
    try {
      ...
      if (isEditing && vehicle) {
        ...
        await base44.entities.Vehicle.create({ ...payload, driver_id: user.id });
  ...
  const handleToggleActive = async () => {
    try {
      await base44.entities.Vehicle.update(vehicle.id, { is_active: !vehicle.is_active });
      setVehicle(prev => ({ ...prev, is_active: !prev.is_active }));
      toast.success(vehicle.is_active ? 'Veículo desativado' : 'Veículo ativado!');
    } catch {
              ...
              <Card className="bg-[#1A1A1A] border-[#F22998]/30 rounded-3xl overflow-hidden">
                ...
                {!vehicle.photo_url && (
                  <div className="w-full h-36 bg-[#F22998]/5 flex items-center justify-center">
                    <Car className="w-16 h-16 text-[#F22998]/30" />
                  </div>
                )}
                ...
                <CardContent className="p-6">
                  ...
                  <div className="mb-1">
                    ...
                    {(vehicle.year || vehicle.color) && (
                      <p className="text-[#F2F2F2]/50 text-sm mt-0.5">
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-6">
                    <Button
                      onClick={openEdit}
                      variant="outline"
                      className="py-5 rounded-2xl border-[#F22998]/30 text-[#F22998] hover:bg-[#F22998]/10"
                    >
          ...
          <AnimatePresence>
            {showForm && (
              <motion.div
                ...
                exit={{ opacity: 0, y: 30 }}
              >
                <Card className="bg-[#1A1A1A] border-[#F22998]/30 rounded-3xl">
                  <CardHeader className="bg-transparent border-b border-[#F22998]/10">
                    <div className="flex items-center justify-between">
                      <CardTitle className="text-[#F2F2F2]">
                        {isEditing ? 'Editar veículo' : 'Cadastrar veículo'}
                      </CardTitle>
                      <Button variant="ghost" size="icon" onClick={() => setShowForm(false)} className="text-[#F2F2F2]/50">
                        <X className="w-5 h-5" />
                      </Button>
                    </div>
                  </CardHeader>
                  <CardContent className="p-6 space-y-5 bg-transparent">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-4">
                        {formData.photo_url ? (
                          <img src={formData.photo_url} alt="Veículo" className="w-24 h-16 object-cover rounded-xl border border-[#F22998]/30" />
                        ) : (
                          <div className="w-24 h-16 rounded-xl border border-dashed border-[#F22998]/30 bg-[#F22998]/5 flex items-center justify-center">
                            <Car className="w-8 h-8 text-[#F22998]/30" />
                          </div>
                        )}
                        <label className="flex items-center gap-2 px-4 py-2 rounded-xl border border-[#F22998]/30 text-[#F22998] text-sm cursor-pointer hover:bg-[#F22998]/10 transition-colors">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        className="bg-[#0D0D0D] border-[#F22998]/20 text-[#F2F2F2] placeholder:text-[#F2F2F2]/30 uppercase"
                        maxLength={8}
                      />
                    </div>
                    ...
                    <div className="flex gap-3 pt-2">
                      <Button
                        ...
                        className="flex-1 py-5 rounded-2xl border-[#F22998]/20 text-[#F2F2F2]/60"
                      >
                        Cancelar
                      </Button>
                      <Button
