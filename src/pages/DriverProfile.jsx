...
import { 
  Car, Shield, Camera, User, Phone, ArrowLeft
} from 'lucide-react';
...
export default function DriverProfile() {
  ...
  const handleTogglePets = async (checked) => {
    try {
      ...
      // Atualizar tag no DriverPresence para filtro do mapa
      const presence = await base44.entities.DriverPresence.filter({ driver_id: user.id });
      if (presence.length > 0) {
        const currentTags = presence[0].tags || [];
        const newTags = checked
          ? [...new Set([...currentTags, 'aceita_pet'])]
          : currentTags.filter(t => t !== 'aceita_pet');
        await base44.entities.DriverPresence.update(presence[0].id, { tags: newTags });
      }
  ...
  return (
    <div className="min-h-screen bg-[#0D0D0D] text-[#F2F2F2] pb-24 md:pb-10">
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        >
          <Card className="glass-effect border-[#F472B6]/30 mb-6 bg-[#0D0D0D]">
            <CardContent className="p-6 bg-transparent">
              <div className="flex flex-col md:flex-row items-center gap-6">
                {/* Photo */}
                <div className="relative">
                  <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-[#F472B6]">
                    {user.photo_url ? (
                  ...
                  <label className="absolute bottom-0 right-0 w-10 h-10 rounded-full bg-[#F472B6] flex items-center justify-center cursor-pointer hover:bg-[#EC4899] transition-colors">
                    <input 
                      type="file" 
                      accept="image/*" 
                      className="hidden" 
                ...
                <div className="flex-1 text-center md:text-left">
                  ...
                  <div className="flex flex-wrap justify-center md:justify-start gap-2">
                    ...
                    {user.accepts_pets && (
                      <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-pink-500/20">
                        <span className="text-sm text-pink-400">🐾 Aceita Pets</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {isEditing && (
                <motion.div
                  ...
                  className="mt-6 pt-6 border-t border-[#F472B6]/20 space-y-4"
                >
                  <div>
        ...
        >
          <Card className="glass-effect border-[#F472B6]/30 mb-6 bg-[#0D0D0D]">
            ...
            <CardContent className="space-y-4 bg-transparent">
              {/* Pets */}
              <div className="flex items-center justify-between p-4 rounded-xl bg-[#F472B6]/5">
