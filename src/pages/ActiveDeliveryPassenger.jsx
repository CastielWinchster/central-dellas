    ...
    return (
      <div className="flex items-center justify-center h-screen bg-[#0D0D0D]">
        <div className="text-center text-white">
          ...
          <button onClick={() => navigate('/RequestDelivery')} className="mt-4 text-[#F472B6]">Voltar</button>
        </div>
      </div>
    );
  }
      ...
      <div className="flex-1 bg-[#111118] rounded-t-3xl -mt-5 relative z-20 overflow-y-auto px-5 pt-5 pb-6">
        ...
        <div className="mb-5">
          <div className="flex items-center justify-between relative">
            ...
            {/* linha de progresso */}
            <div
              className="absolute top-4 left-4 h-0.5 bg-[#F472B6] transition-all duration-500"
              ...
              return (
                <div key={step.key} className="flex flex-col items-center gap-1 z-10">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm transition-all ${
                    done ? 'bg-[#F472B6]' : active ? 'bg-[#F472B6]/30 border-2 border-[#F472B6]' : 'bg-white/10'
                  } ${active ? 'animate-pulse' : ''}`}>
                    {step.icon}
                  </div>
                  <span className={`text-[9px] font-medium text-center max-w-[52px] leading-tight ${active ? 'text-[#F472B6]' : done ? 'text-white/70' : 'text-white/30'}`}>
                    {step.label}
                  </span>
                </div>
              );
        ...
        {driver && (
          <div className="flex items-center gap-4 mb-4">
            <div className="w-14 h-14 rounded-full border-2 border-[#F472B6] overflow-hidden flex-shrink-0 bg-gradient-to-br from-[#EC4899] to-[#BE185D] flex items-center justify-center">
              {driver.photo
        ...
        {ride.status === 'delivered' && (
          <div className="text-center py-4">
            ...
            <button onClick={() => navigate('/RequestDelivery')} className="mt-3 text-[#F472B6] text-sm">Fazer nova entrega</button>
          </div>
        )}
      </div>
