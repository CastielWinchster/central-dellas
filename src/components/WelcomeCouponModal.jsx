      ...
      {show && (
        ...
        >
          <motion.div
            ...
            className="relative max-w-md w-full bg-[#1a1a1a]/95 backdrop-blur-xl rounded-3xl border border-[#F472B6]/30 overflow-hidden"
          >
            {/* Decoração de fundo */}
            <div className="absolute top-0 right-0 w-40 h-40 bg-[#F472B6]/10 rounded-full blur-3xl pointer-events-none" />
            <div className="absolute bottom-0 left-0 w-32 h-32 bg-[#EC4899]/10 rounded-full blur-3xl pointer-events-none" />

            {/* Botão fechar */}
            <button
              onClick={dismiss}
              className="absolute top-4 right-4 z-10 w-8 h-8 rounded-full bg-[#F472B6]/10 hover:bg-[#F472B6]/20 flex items-center justify-center transition-colors"
            >
              <X className="w-4 h-4 text-[#F472B6]" />
            </button>

            <div className="relative z-10 p-8 text-center">
              <div className="w-16 h-16 rounded-full bg-gradient-to-br from-[#EC4899] to-[#F472B6] flex items-center justify-center mx-auto mb-4">
              ...
              {/* Card do cupom */}
              <div className="my-6 p-4 rounded-2xl bg-[#F472B6]/10 border border-[#F472B6]/30">
                <p className="text-sm text-[#F2F2F2]/60 mb-2">Cupom de Desconto</p>
                <div className="flex items-center justify-center gap-2">
                  <p className="text-3xl font-bold text-[#F472B6] tracking-wide">
                    {coupon.code.toUpperCase()}
                  </p>
                  <button
                    onClick={() => { navigator.clipboard.writeText(coupon.code).catch(() => {}); toast.success('Código copiado!'); }}
                    className="p-1.5 rounded-lg bg-[#F472B6]/20 hover:bg-[#F472B6]/30 transition-colors"
                  >
