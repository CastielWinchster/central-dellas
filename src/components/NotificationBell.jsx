...
const TYPE_ICON = {
  ride:    <Car size={16} className="text-[#F472B6]" />,
  ...
  return (
    <DropdownMenu open={open} onOpenChange={handleOpen}>
      <DropdownMenuTrigger asChild>
        <button className="relative p-2 rounded-lg hover:bg-[#F472B6]/10 transition-colors">
          <Bell className="w-6 h-6 text-[#F2F2F2]" />
          {unreadCount > 0 && (
            <motion.span
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              className={`absolute -top-1 -right-1 w-5 h-5 bg-[#F472B6] text-white text-xs rounded-full flex items-center justify-center font-bold ${badgePop ? 'badge-pop' : ''}`}
            >
      ...
      <DropdownMenuContent
        align="end"
        className="w-96 bg-[#0D0D0D] border-[#F472B6]/30 p-0 max-h-[500px] overflow-hidden"
      >
        <div className="p-4 border-b border-[#F472B6]/20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            {unreadCount > 0 && (
              <Button
                ...
                variant="ghost"
                size="sm"
                className="text-[#F472B6] hover:bg-[#F472B6]/10 text-xs"
              >
                Marcar lidas
              </Button>
            )}
        ...
        <div className="overflow-y-auto max-h-[400px]">
          {notifications.length > 0 ? (
              ...
              {notifications.map((notification) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`p-4 border-b border-[#F472B6]/10 cursor-pointer transition-colors hover:bg-[#F472B6]/5 ${
                      !notification.is_read ? 'bg-[#F472B6]/10' : ''
                    }`}
                  >
            ...
            <div className="p-8 text-center">
              <Bell className="w-12 h-12 text-[#F472B6]/30 mx-auto mb-3" />
        ...
        {notifications.length > 0 && (
          <div className="p-3 border-t border-[#F472B6]/20">
            <Link to={createPageUrl('Notifications')}>
              <Button
                variant="ghost"
                className="w-full text-[#F472B6] hover:bg-[#F472B6]/10"
