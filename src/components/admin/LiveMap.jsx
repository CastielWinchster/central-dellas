...
function driverMarkerEl(busy) {
  ...
  el.style.cssText = `
    ...
    background: ${busy ? 'linear-gradient(135deg,#10b981,#059669)' : 'linear-gradient(135deg,#EC4899,#F472B6)'};
    border: 3px solid #fff; display:flex; align-items:center; justify-content:center;
    box-shadow: 0 2px 12px ${busy ? 'rgba(16,185,129,0.6)' : 'rgba(244,114,182,0.5)'};
...
export default function LiveMap() {
  ...
  const summary = [
    ...
    { icon: Car, label: 'Disponíveis', value: idleCount, color: 'text-[#F472B6]', bg: 'bg-[#F472B6]/20' },
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-3">
        {summary.map((s, i) => (
          <Card key={i} className="bg-[#F2F2F2]/5 border-[#F472B6]/10 p-4">
      ...
      <Card className="overflow-hidden rounded-2xl border-[#F472B6]/10">
        <div ref={mapContainer} style={{ height: '360px', width: '100%' }} />
      </Card>

      {!loading && drivers.length === 0 && (
        <Card className="bg-[#F2F2F2]/5 border-[#F472B6]/10 p-6 text-center">
          <Radio className="w-10 h-10 text-[#F472B6]/40 mx-auto mb-2" />
