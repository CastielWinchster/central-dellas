...
export default function CouponManager() {
  ...
  return (
    <div className="space-y-4">
      {/* Form */}
      <Card className="bg-[#F2F2F2]/5 border-[#F472B6]/10 p-5">
        <h3 className="font-bold text-[#F2F2F2] mb-4 flex items-center gap-2">
          <Plus className="w-5 h-5 text-[#F472B6]" /> Novo Cupom
        </h3>
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <Select value={form.discount_type} onValueChange={(v) => setForm({ ...form, discount_type: v })}>
              <SelectTrigger className="bg-[#0D0D0D] border-[#F472B6]/20 text-white"><SelectValue /></SelectTrigger>
        ...
        <Button
          ...
          disabled={saving}
          className="w-full mt-4 bg-gradient-to-r from-[#EC4899] to-[#F472B6] hover:opacity-90 text-white font-bold"
        >
      ...
      <Card className="bg-[#F2F2F2]/5 border-[#F472B6]/10 p-5">
        <h3 className="font-bold text-[#F2F2F2] mb-4 flex items-center gap-2">
          <Ticket className="w-5 h-5 text-[#F472B6]" /> Cupons ({coupons.length})
        </h3>
        {loading ? (
          ...
          <div className="space-y-2">
            {coupons.map((c) => (
              <div key={c.id} className="flex items-center justify-between bg-[#0D0D0D] rounded-xl p-4 border border-[#F472B6]/10">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg bg-[#F472B6]/15 flex items-center justify-center">
                    {c.discount_percentage ? <Percent className="w-5 h-5 text-[#F472B6]" /> : <DollarSign className="w-5 h-5 text-[#F472B6]" />}
                  </div>
                  <div>
