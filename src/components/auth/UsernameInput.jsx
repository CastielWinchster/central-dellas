...
export default function UsernameInput({ value, onChange }) {
  ...
  const getBorderColor = () => {
    if (!value) return 'border-[#F472B6]/30';
  ...
  const getIcon = () => {
    ...
    return <User className="w-5 h-5 text-[#F472B6]" />;
  };
    ...
    <div>
      ...
      <div className="relative">
        ...
        <Input
          ...
          className={`pl-10 bg-[#0D0D0D] ${getBorderColor()} text-[#F2F2F2] focus:border-[#F472B6]`}
          required
          maxLength={20}
        />
      </div>
          ...
          <div className="flex flex-wrap gap-2">
            {suggestions.map((suggestion) => (
              <Button
                ...
                className="text-xs border-[#F472B6]/30 text-[#F472B6] hover:bg-[#F472B6]/10"
              >
                {suggestion}
              </Button>
            ))}
